  ...
  return (
    <section className="py-20 bg-[#F9FAFB]">
      <div className="container mx-auto px-4">
            ...
            <motion.button
              ...
              className={`px-6 py-3 rounded-full border border-gray-300 shadow-lg group overflow-hidden transition-all duration-300 relative ${
                activeCategory === category
                  ...
                  : 'bg-white text-[#2D5C76] hover:bg-[#2D5C76] hover:text-white'
              }`}
            >
          ...
          {filteredTechnologies.map((tech, index) => (
            ...
            >
              <div className="flex items-start space-x-4">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-[#2D5C76] mb-1">{tech.name}</h3>
                  <p className="text-sm text-gray-600">{tech.description}</p>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-gray-50 ">
                <span className="inline-block px-3 py-1 bg-[#F9FAFB] border border-gray-300 text-[#2D5C76] text-sm rounded-full">
                  {tech.category}
                </span>
              </div>
            </motion.div>
        ...
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-[#2D5C76]/5 rounded-full blur-3xl -z-10"></div>
      </div>
    </section>
  )
}
